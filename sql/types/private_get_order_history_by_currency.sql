...
comment on column deribit.private_get_order_history_by_currency_response_result.amount is 'It represents the requested order size. For perpetual and futures the amount is in USD units, for options it is amount of corresponding cryptocurrency contracts, e.g., BTC or ETH.';
...
create type deribit.private_get_order_history_by_currency_request_kind as enum ('any', 'combo', 'future_combo', 'option', 'future', 'option_combo', 'spot');
