drop type if exists deribit.private_edit_by_label_response_leg cascade;
create type deribit.private_edit_by_label_response_leg as (
	liquidation text,
	liquidity text,
	mark_price float,
	matching_id text,
...
comment on column deribit.private_edit_by_label_response_leg.liquidation is 'Optional field (only for trades caused by liquidation): "M" when maker side of trade was under liquidation, "T" when taker side was under liquidation, "MT" when both sides of trade were under liquidation';
comment on column deribit.private_edit_by_label_response_leg.liquidity is 'Describes what was role of users order: "M" when it was maker order, "T" when it was taker order';
comment on column deribit.private_edit_by_label_response_leg.mark_price is 'Mark Price at the moment of trade';
comment on column deribit.private_edit_by_label_response_leg.matching_id is 'Always null';
comment on column deribit.private_edit_by_label_response_leg.mmp is 'true if user order is MMP';
comment on column deribit.private_edit_by_label_response_leg.order_id is 'Id of the user order (maker or taker), i.e. subscriber''s order id that took part in the trade';
comment on column deribit.private_edit_by_label_response_leg.order_type is 'Order type: "limit, "market", or "liquidation"';
comment on column deribit.private_edit_by_label_response_leg.post_only is 'true if user order is post-only';
comment on column deribit.private_edit_by_label_response_leg.price is 'Price in base currency';
comment on column deribit.private_edit_by_label_response_leg.profit_loss is 'Profit and loss in base currency.';
comment on column deribit.private_edit_by_label_response_leg.reduce_only is 'true if user order is reduce-only';
comment on column deribit.private_edit_by_label_response_leg.risk_reducing is 'true if user order is marked by the platform as a risk reducing order (can apply only to orders placed by PM users)';
comment on column deribit.private_edit_by_label_response_leg.state is 'Order state: "open", "filled", "rejected", "cancelled", "untriggered" or "archive" (if order was archived)';
comment on column deribit.private_edit_by_label_response_leg.tick_direction is 'Direction of the "tick" (0 = Plus Tick, 1 = Zero-Plus Tick, 2 = Minus Tick, 3 = Zero-Minus Tick).';
comment on column deribit.private_edit_by_label_response_leg.timestamp is 'The timestamp of the trade (milliseconds since the UNIX epoch)';
comment on column deribit.private_edit_by_label_response_leg.trade_id is 'Unique (per currency) trade identifier';
comment on column deribit.private_edit_by_label_response_leg.trade_seq is 'The sequence number of the trade within instrument';
comment on column deribit.private_edit_by_label_response_leg.underlying_price is 'Underlying price for implied volatility calculations (Options only)';
...
create type deribit.private_edit_by_label_response_trade as (
	...
	iv float,
	label text,
	legs deribit.private_edit_by_label_response_leg[]
);
...
comment on column deribit.private_edit_by_label_response_trade.amount is 'Trade amount. For perpetual and futures - in USD units, for options it is amount of corresponding cryptocurrency contracts, e.g., BTC or ETH.';
...
comment on column deribit.private_edit_by_label_response_order.cancel_reason is 'Enumerated reason behind cancel "user_request", "autoliquidation", "cancel_on_disconnect", "risk_mitigation", "pme_risk_reduction" (portfolio margining risk reduction), "pme_account_locked" (portfolio margining account locked per currency), "position_locked", "mmp_trigger" (market maker protection), "edit_post_only_reject" (cancelled on edit because of reject_post_only setting)';
...
comment on column deribit.private_edit_by_label_response_order.amount is 'It represents the requested order size. For perpetual and futures the amount is in USD units, for options it is amount of corresponding cryptocurrency contracts, e.g., BTC or ETH.';
...
create type deribit.private_edit_by_label_request_advanced as enum ('implv', 'usd');
