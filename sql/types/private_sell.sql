...
comment on column deribit.private_sell_response_order.cancel_reason is 'Enumerated reason behind cancel "user_request", "autoliquidation", "cancel_on_disconnect", "risk_mitigation", "pme_risk_reduction" (portfolio margining risk reduction), "pme_account_locked" (portfolio margining account locked per currency), "position_locked", "mmp_trigger" (market maker protection), "edit_post_only_reject" (cancelled on edit because of reject_post_only setting)';
...
comment on column deribit.private_sell_response_order.amount is 'It represents the requested order size. For perpetual and futures the amount is in USD units, for options it is amount of corresponding cryptocurrency contracts, e.g., BTC or ETH.';
...
create type deribit.private_sell_request_type as enum ('limit', 'market', 'market_limit', 'stop_limit', 'stop_market', 'take_limit', 'take_market', 'trailing_stop');

drop type if exists deribit.private_sell_request_time_in_force cascade;
create type deribit.private_sell_request_time_in_force as enum ('fill_or_kill', 'good_til_cancelled', 'good_til_day', 'immediate_or_cancel');

drop type if exists deribit.private_sell_request_trigger cascade;
create type deribit.private_sell_request_trigger as enum ('index_price', 'last_price', 'mark_price');

drop type if exists deribit.private_sell_request_advanced cascade;
create type deribit.private_sell_request_advanced as enum ('implv', 'usd');
