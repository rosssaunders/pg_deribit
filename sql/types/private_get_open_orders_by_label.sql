drop type if exists deribit.private_get_open_orders_by_label_response_result cascade;
...
comment on column deribit.private_get_open_orders_by_label_response_result.amount is 'It represents the requested order size. For perpetual and futures the amount is in USD units, for options it is amount of corresponding cryptocurrency contracts, e.g., BTC or ETH.';
...
drop type if exists deribit.private_get_open_orders_by_label_response cascade;
